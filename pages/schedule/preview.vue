          ...
          <view class="page-section-spacing">
            ...
            >
              <swiper-item v-for="(item,index) in (days.length)/7" :key="index">
                <view
                  style="width:92rpx;height:306rpx;display:inline-block;border:1px solid #f0f0f0;text-align:center;padding-top:30rpx"
                  v-for="item in days.slice(index*7,(index+1)*7)"
                  :key="item.day.toString()"
                  class="swiper-item"
                  @tap="tapDay"
                  :class="{today:formatDay(item.day)==today&&item.day.getMonth()+1==getMonth(selectMonth)}"
                >{{item.day.getDate()}}</view>
              </swiper-item>
            </swiper>
          </view>
        </view>
    ...
    <div class="timeInfo">
      ...
      <div class="info">
        <ul v-for="(item,index) in dayNum" :key="index">
          <li ref="liItem" v-for="(item,index55) in timeList" :key="index55">
            <span></span>
          </li>

          <div
            v-for="(item2,index2) in todoOneDay"
            v-if="item2.day==days.slice(current*7,(current+1)*7)[index].day.getDate()"
            :key="index2+'a'"
          >
            <p
              v-if="item3"
              class="cursor"
              :style="'margin-top:'+item3.top+'px'+';'+'height:'+item3.height+'px'"
          ...
          <div
            v-for="(item2,index2) in todoOneDay"
            v-if="item2.day==days.slice(current*7,(current+1)*7)[index].day.getDate()"
            :key="item2.day+index2"
          >
            <div v-for="(item3,index3) in item2.info2" :key="item3.info+index3">
              <p
                class="cursor"
                v-if="item3"
                :style="'margin-top:'+item3.top+'px'+';'+'height:'+item3.height+'px'"
...
export default {
  ...
  methods: {
    getTodo(start, end) {
      //获取代办
      this.$http
        .get(calendar.getByDay, { start_time: start, end_time: end })
        .then(res => {
          console.log(res);
            ...
            if (item.start_time.split(" ")[0] == item.end_time.split(" ")[0]) {
              ...
              this.todoOneDay.push({
                ...
                info: [
                  {
                    info: item.name,
                    time: `${this.getHour(item.start_time)}-${this.getHour(
                      item.end_time
                    )}`
                  }
                ]
              ...
              // this.todoMulDay.push()
              this.todoMulDay.push({
                left:0,
                info:[
                  {
                    info:item.name,
                    time:`${item.start_time.split(' ')[0]}-${item.end_time.split(' ')[0]}`
                  }
                ]
              })
            }
    ...
    animationfinish(e) {
      ...
      if (e.detail.current + 1 == this.days.length / 7) {
        //右边滑到底
      }
      if (e.detail.current == 0) {
        //左边滑到底
        // this.days.unshift(...this.days.slice(0, 7));
      }
