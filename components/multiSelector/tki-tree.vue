						...
						<view class="tki-tree-item" :style="[{
						 ...
						 :class="{
							...
							<view class="tki-tree-label" @tap.stop="_treeItemTap(item, index)">
								...
								{{item.name}}
								<text v-if="!item.lastRank">({{item.count}})</text>
							</view>
			...
			_renderTreeList(list = [], rank = 0, parentId = [], parents = []) {
				list.forEach(item => {
					this.treeList.push({
						...
						parents, // 父级id数组
						rank, // 层级
						showChild: parentId.length<=0?true:false, //子级是否显示
						open: false, //是否打开
						show: rank === 0, // 自身是否显示
						hideArr: [],
					...
					if (Array.isArray(item.children) && item.children.length > 0) {
						...
						console.log('this.treeList[this.treeList.length - 1]----------',this.treeList[this.treeList.length - 1]);
						if(this.treeList[this.treeList.length - 1].parentId.length>0){
							this.treeList[this.treeList.length - 1].lastRank = true;
						}
						
					}
				})
				console.log('+++++++++++++++++++tki----------tre',this.treeList);
			},
			// 处理默认选择
			_defaultSelect() {
				this.treeList.forEach((v, i) => {
